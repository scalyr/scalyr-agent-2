...
def print_numa_stats(numafiles):
    ...
    for numafile in numafiles:
        # Reopen file instead of seeking to avoid seekability issues
        numafile_filename = numafile.name
        node_id = int(numafile_filename[numafile_filename.find("/node/node") + 10 : -9])
        ts = int(time.time())
        with open(numafile_filename) as f:
...
def main():
    ...
    f_uptime_filename = "/proc/uptime"
    f_meminfo_filename = "/proc/meminfo"
    f_vmstat_filename = "/proc/vmstat"
    f_stat_filename = "/proc/stat"
    f_loadavg_filename = "/proc/loadavg"
    f_entropy_avail_filename = "/proc/sys/kernel/random/entropy_avail"
    ...
    while True:
        ...
        # proc.uptime
        with open(f_uptime_filename) as f_uptime:
            ts = int(time.time())
        ...
        # proc.meminfo
        with open(f_meminfo_filename) as f_meminfo:
            ts = int(time.time())
        ...
        # proc.vmstat
        with open(f_vmstat_filename) as f_vmstat:
            ts = int(time.time())
        ...
        # proc.stat
        with open(f_stat_filename) as f_stat:
            ts = int(time.time())
            for line in f_stat:
        ...
        with open(f_loadavg_filename) as f_loadavg:
            ts = int(time.time())
        ...
        with open(f_entropy_avail_filename) as f_entropy_avail:
            ts = int(time.time())
