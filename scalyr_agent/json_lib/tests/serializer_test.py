...
class SerializeTests(ScalyrTestCase):
    def test_numbers(self):
        self.assertEqual(self.write(1), '1')
        self.assertEqual(self.write(1.2), '1.2')
        self.assertEqual(self.write(133L), '133')

    def test_bool(self):
        self.assertEqual(self.write(True), 'true')
        self.assertEqual(self.write(False), 'false')

    def test_4byte_utf8_fast(self):
        actual = '\xF0\xAA\x9A\xA5'
        expected_fast = '"\xf0\xaa\\u009a\xa5"'
        self.assertEqual(serialize(actual, use_fast_encoding=True), expected_fast)
    ...
    def test_4byte_utf8_slow(self):
        ...
        self.assertEqual(serialize(actual, use_fast_encoding=False), expected_slow)
    ...
    def test_string_fast(self):
        ...
        self.assertEqual(serialize('Escaped\xE2\x82\xAC', use_fast_encoding=True), '"Escaped\xe2\\u0082\xac"')
    ...
    def test_string_slow(self):
        ...
        self.assertEqual(serialize('Escaped\xE2\x82\xAC', use_fast_encoding=False), '"Escaped\\u20ac"')

    def test_dict(self):
        self.assertEqual(self.write({'hi': 5}), '{"hi":5}')
        self.assertEqual(self.write({'bye': 5, 'hi': True}), '{"bye":5,"hi":true}')
        self.assertEqual(self.write({'bye': 5, 'hi': {'foo': 5}}), '{"bye":5,"hi":{"foo":5}}')
        self.assertEqual(self.write({}), '{}')

    def test_array(self):
        self.assertEqual(self.write([1, 2, 5]), '[1,2,5]')
        self.assertEqual(self.write([]), '[]')

    def test_length_prefixed_strings(self):
        self.assertEqual('`s\x00\x00\x00\x0cHowdy folks!', serialize('Howdy folks!', use_length_prefix_string=True))

    def test_length_prefixed_strings_with_unicode(self):
        self.assertEqual('`s\x00\x00\x00\x10Howdy \xe8\x92\xb8 folks!', serialize(u'Howdy \u84b8 folks!',
    ...
    def __run_string_test_case(self, input_string, expected_result):
        self.assertEqual(serialize(input_string, use_fast_encoding=True), expected_result)
        self.assertEqual(serialize(input_string, use_fast_encoding=False), expected_result)
