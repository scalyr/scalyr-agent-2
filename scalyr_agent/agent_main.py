...
class ScalyrAgent(object):
    ...
    def __condrestart(self, quiet, no_fork, no_check_remote):
        ...
        if self.__controller.is_agent_running():
            ...
            if self.__stop(quiet) != 0:
                log.error('Failed to stop the running agent.  Cannot restart until it is killed.')
                return 1
    ...
    def __fail_if_already_running(self):
        ...
        try:
            ...
            log.error('Failed to start agent because it is already running.')
            log.error('%s' % str(e))
            sys.exit(4)
    ...
    def __create_client(self, quiet=False):
        ...
        return ScalyrClientSession(self.__config.scalyr_server, self.__config.api_key, SCALYR_VERSION, quiet=quiet,
                                   ...
                                   ca_file=ca_file, intermediate_certs_file=intermediate_certs_file,
                                   use_requests_lib=use_requests_lib, use_tlslite=use_tlslite, compression_type=self.__config.compression_type,
