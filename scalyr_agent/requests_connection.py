...
class RequestsConnection( Connection ):
    ...
    def _post( self, request_path, body ):
        ...
        self.__response = self.__session.post( url, verify=self.__verify(), timeout=self._request_deadline, data=body )

    def _get( self, request_path ):
        url = self._full_address + request_path
        self.__check_session()
        self.__response = self.__session.get( url, verify=self.__verify(), timeout=self._request_deadline )
