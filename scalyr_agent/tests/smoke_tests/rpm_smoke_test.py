...
CMD python -m pytest /agent_source/scalyr_agent/tests/smoke_tests/agent_standalone_smoke_test.py --config config.ini --runner_type PACKAGE

    """


def build_rpm_agent_image(image_tag, python_version):
...
def test_rpm_agent(request):
    ...
    build_package_builder_image("scalyr_test_agent_package_builder")
    build_rpm_builder_image("scalyr_test_agent_rpm_builder")

    rpm_agent_image_tag = "scalyr_test_agent_rpm_image_{}".format(python_version)
    build_rpm_agent_image(rpm_agent_image_tag, python_version)
