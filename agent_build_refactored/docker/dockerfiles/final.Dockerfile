...
RUN AGENT_BUILD_IN_DOCKER=1 python3 /scalyr-agent-2/agent_build_refactored/scripts/runner_helper.py ${BUILDER_FQDN} build-container-filesystem --output /tmp/container-fs
...
# Build stage for image type which does not need any additional preparations..
...
# Final result stage with command.
