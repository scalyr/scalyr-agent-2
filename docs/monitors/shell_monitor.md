...
|||# ``extract``       ||| Optional: a regular expression to apply to the command output. If defined, this expression \
                           must contain a matching group (i.e. a subexpression enclosed in parentheses). The monitor \
                           will record only the content of that matching group. This allows you to discard unnecessary \
                           portions of the command output and extract the information you need.
...
|||# ``max_characters``||| Optional (defaults to 200). At most this many characters of output are recorded. You may \
                           specify a value up to 10000, but the Scalyr server currently truncates all fields to 3500 \
                           characters.
