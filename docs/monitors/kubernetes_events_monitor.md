...
| `leader_candidate_label` | Optional (defaults to `agent.config.scalyr.com/events_leader_candidate=true`). If `check_labels` is true, then the monitor will check for any nodes with the label configured using this option and the node with this label set and that has the oldest creation time will be the event monitor leader. | 
