...
|||# ``id``            ||| Optional. Included in each log message generated by this monitor, as a field named \
                           ...
                           This is especially useful if you are running multiple Apache instances on a single server; \
                           you can monitor each instance with a separate apache_monitor record in the Scalyr Agent \
                           configuration.
|||# ``status_url``    ||| Optional.  Specifies the URL -- in particular, the port number -- at which the Apache \
                           status module is served.  The URL should end in ``/?auto`` to indicate the machine-readable \
                           version of the page should be returned.
