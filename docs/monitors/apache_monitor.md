...
| `status_url`     | Optional.  Specifies the URL -- in particular, the port number -- at which the Apache status module is served.  The URL should end in ``/?auto`` to indicate the machine-readable version of the page should be returned. | 
