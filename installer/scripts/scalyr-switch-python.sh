...
if [ $# -lt 1 ]; then
  echo "Usage: ${0} <default|python2|python3|embedded>"
  exit 2
fi
...
if [[ ${1} == "--help" || ${1} == "-h" ]]; then
  ...
  echo "'embedded' - use python interpreter which is shipped with agent package"

  exit 0
fi

if [  "${1}" != "default" ] && [  "${1}" != "python2" ] && [ "${1}" != "python3" ] && [ "${1}" != "embedded" ]; then
...
if [ "${EXECUTABLE_NAME}" == "embedded" ]; then
  /usr/lib/scalyr-agent-2-dependencies/bin/python3 /usr/share/scalyr-agent-2/bin/scalyr-agent-2 config --set-python="embedded"
else
