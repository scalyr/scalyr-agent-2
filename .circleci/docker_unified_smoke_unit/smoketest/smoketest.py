...
class K8sActor(DockerSmokeTestActor):
    ...
    def verify_logs_uploaded(self):
        ...
        def _query_scalyr_for_metrics(metric_name):
            def _func():
                resp = requests.get(
                    self._make_query_url(
                        {"metric": metric_name},
        ...
        for count, metric in enumerate(metrics_to_check):
            self.poll_until_max_wait(
                _query_scalyr_for_metrics(metric),
                "Querying server to verify upload of metric: {}".format(metric),
                "Upload verified for {}.".format(metric),
                "Upload not verified for {}.".format(metric),
