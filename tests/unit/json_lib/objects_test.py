...
class JsonObjectTests(ScalyrTestCase):
    ...
    def test_get_long(self):
        x = JsonObject(foo=5)
        self.assertEqual(x.get_long("foo"), 5)
        ...
        # Test the default value is returned if field is missing.
        self.assertEqual(x.get_long("none", default_value=5), 5)
    ...
    def test_to_dict(self):
        x = JsonObject(foo="a", bar=10)
        self.assertEqual(dict(foo="a", bar=10), x.to_dict())

        x = JsonObject(foo=JsonObject(bee=1), bar=10)
        self.assertEqual(dict(foo=dict(bee=1), bar=10), x.to_dict())

        x = JsonObject(foo=dict(bee=1, boo=JsonObject(hi=True)), bar=10)
        self.assertEqual(dict(foo=dict(bee=1, boo=dict(hi=True)), bar=10), x.to_dict())

        x = JsonObject(foo=JsonArray(1, 2, 3), bar=10)
        self.assertEqual(dict(foo=[1, 2, 3], bar=10), x.to_dict())

        x = JsonObject(foo=[1, 2, JsonObject(foo=5)], bar=10)
        self.assertEqual(dict(foo=[1, 2, dict(foo=5)], bar=10), x.to_dict())
