...
from scalyr_agent.copying_manager.copying_manager import LogPathsBeingProcessed
...
class TestLogLineRedactor(ScalyrTestCase):
    ...
    def test_multiple_regular_expression_AGENT_140(self):
        ...
        self._run_case(
            ...
            + "%s %s %s"
            % (
                md5_hexdigest("xxx.yyy"),
                md5_hexdigest("aaa.bbb"),
                md5_hexdigest("com"),
            )
            + trail_text,
            True,
...
class TestLogMatcher(ScalyrTestCase):
    ...
    def test_matches_glob(self):
        ...
        processors = matcher.find_matches("WORKER_ID_0",  LogPathsBeingProcessed(), dict())
    ...
    def test_is_finished_processors_not_finished(self):
        ...
        matcher = LogMatcher(self.__config, config)
        matcher.finish()
        processors = matcher.find_matches("WORKER_ID_0",  LogPathsBeingProcessed(), dict(), copy_at_index_zero=True)
    ...
    def test_find_matches_if_completely_finished(self):
        ...
        new_processors = matcher.find_matches("WORKER_ID_0",  LogPathsBeingProcessed(), dict(), copy_at_index_zero=True)
