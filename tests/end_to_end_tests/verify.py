...
def verify_agent_status(
    ...
    while True:
        ...
        if copying_manager_status is None:
            timeout_tracker.sleep(1, "Can't wait further for agent's copying_manager_status.")
            continue

        last_response_status = copying_manager_status.get("last_response_status")
        if last_response_status is None:
            timeout_tracker.sleep(1, "Can't wait further for agent's copying manager last_response_status.")
            continue

        assert last_response_status == "success", "last_response_status is not successful"
        break
