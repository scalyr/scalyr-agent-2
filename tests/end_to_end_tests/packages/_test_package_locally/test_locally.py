...
from tests.end_to_end_tests.log_verify import verify_test_log_file_upload, \
    check_agent_log_for_errors, write_messages_to_test_log, check_requests_stats_in_agent_log
...
def _get_agent_version():
    ...
    return re.search(r"The Scalyr Agent 2 version is (\d+\.\d+\.\d+)", version_output).group(1)
...
def _verify_package_files_ownership(
        package_type: str,
    ...
    """
    if package_type == "rpm":
        files_permissions = subprocess.check_output([
            "rpm", "-qplv", str(package_path)]
        ...
        assert agent_config_stats[2] == "root" and agent_config_stats[3] == "root", "Owner of the file is not root:root!"
        return

    raise AssertionError(f"Unknown package type {package_type}.")


def _verify_agent_status(
        package_version: str
):
...
def _verify_system_after_installation(
    package_type: str,
    agent_paths: AgentPaths
):
    ...
    assert agent_config_permissions_str == "640", f"Expected 640 permissions for agent.json, got {agent_config_permissions_str}."

    agent_d_permissions = oct(agent_paths.agent_d_path.stat().st_mode)
    agent_d_permissions_str = str(agent_d_permissions)[-3:]

    assert agent_d_permissions_str in ["741", "751"], f"Expected 741/751 permissions for agent.d, got ${agent_d_permissions_str}."
...
def _restart_agent_and_remove_logs(
        agent_paths: AgentPaths
):
    log.info("Restarting agent...")
    _stop_agent(
...
def _stop_agent(
    ...
    assert not agent_paths.pid_file.exists(), "Agent has to be stopped, but its PID file still exists."
...
def _perform_ssl_checks(
    ...
    use_config(
        test_config_name="agent.json_invalid_ca_cert_path_linux"
    )
    ...
    assert err_info.value.returncode == 1, "scalyr-agent-2 status -v command should have exited with 1 exit code"
    ...
    assert 'Invalid path "/tmp/invalid/ca_certs.crt" specified for the "ca_cert_path"' in log_content
    ...
    getent_output = subprocess.check_output(
    ...
    assert "Last copy response status:                 client/connectionFailedCertHostnameValidationFailed" in status
    ...
    assert "Last copy response size:                   0" in status
    assert "Last copy response status:                 client/connectionFailedSSLError" in status
...
def test(
    ...
    assert _get_agent_version() == package_version, "Version of the installed agent does not match with the target " \
                                                    "version."
    ...
    config = {
        ...
        "server_attributes": {
            "serverHost": test_session_id
        },
        "logs": [
            {
                "path": str(upload_test_log_path),
    ...
    assert _get_agent_version() == previous_package_version, "Version of the downgraded agent does not match"
    ...
    assert _get_agent_version() == package_version, "Version of the upgraded agent does not match with the target " \
                                                    "version."
    _start_and_wait_agent()
