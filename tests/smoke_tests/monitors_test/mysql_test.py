...
def _test(
    ...
    try:
        ...
        if (
            ...
            or expected_exception not in str(e)
            or not re.search(expected_exception, str(e))
        ):
            raise e
...
def test_mysql_python2_ssl_bad_hostname(request):
    _test(
        ...
        expected_exception="hostname '127.0.0.1' doesn't match 'MySQL_Server_5.7.\d+_Auto_Generated_Server_Certificate'",
    )
