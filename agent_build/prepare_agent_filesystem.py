...
def _add_certs(
    ...
    source_certs_path = agent_build.tools.common.SOURCE_ROOT / "certs"
...
def get_build_info():
    ...
    try:
        ...
        os.chdir(agent_build.tools.common.SOURCE_ROOT)
...
def build_agent_base_files(
    ...
    shutil.copytree(agent_build.tools.common.SOURCE_ROOT / "monitors", monitors_path)
    ...
    # Add VERSION file.
    shutil.copy2(
        agent_build.tools.common.SOURCE_ROOT / "VERSION", output_path / "VERSION"
    )
    ...
    if copy_agent_source:
        ...
        shutil.copytree(
            agent_build.tools.common.SOURCE_ROOT / "scalyr_agent", source_code_path / "scalyr_agent"
        )
...
def build_linux_agent_files(
    ...
    for f in ["Dockerfile.custom_agent_config", "Dockerfile.custom_k8s_config"]:
        shutil.copy2(agent_build.tools.common.SOURCE_ROOT / "docker" / f, misc_path / f)
