...
from agent_build.tools import constants, runner
...
class ExampleStep(runner.ShellScriptRunnerStep):
    @property
...
def example_deployment(request):
    ...
    yield deployment
    runner.ALL_DEPLOYMENTS.pop(name)
...
def test_example_deployment(example_deployment: runner.Deployment, in_ci_cd: bool):
    ...
    if example_deployment.in_docker:
        subprocess.check_call(
            ["docker", "image", "rm", "-f", example_deployment.result_image]
        )
    ...
    with mock.patch.object(runner, "save_docker_image", step_save_image_mock):
...
def test_deployment_step_with_untracked_file(caplog, capsys):
    ...
    class ExampleInvalidStepWithUntrackedFiles(runner.ShellScriptRunnerStep):
        @property
        def script_path(self) -> pl.Path:
    ...
    try:
        with pytest.raises(runner.DeploymentStepError):
            deployment.deploy()
        ...
        runner.ALL_DEPLOYMENTS.pop("example_environment_untracked")
